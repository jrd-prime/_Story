...
namespace _StoryGame.Game.Managers.Condition
{
    public sealed class ConditionChecker
    {
        ...
        public ESwitchState GetSwitchState(EGlobalCondition impactOnCondition) =>
            _conditionRegistry.IsCompleted(impactOnCondition) ? ESwitchState.On : ESwitchState.Off;

        public bool GetConditionState(EGlobalCondition condition) =>
            _conditionRegistry.IsCompleted(condition);

        public bool IsInteractBlocked(BlockingCondition[] blockingConditions) =>
            blockingConditions
                .Any(condition => _conditionRegistry.GetConditionState(condition.type) == condition.value);
    }
}
